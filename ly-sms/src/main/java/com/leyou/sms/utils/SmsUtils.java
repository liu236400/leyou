...
@Slf4j
@Component
@EnableConfigurationProperties(SmsProperties.class)//读取配置类
    ...
    public SendSmsResponse sendSms(String phone,String signName, String templateCode,Map<String,Object> params) {
        ...
        try {
            ...
            if (!resp.getCode().equals("OK")) {
                //消息发送未成功
                log.info("【消息服务】消息发送未成功，手机号码：{}，原因：{}", phone, resp.getMessage());
                return null;
            }

            //存入Redis中，设置失效时间为1min  String.valueOf(System.currentTimeMillis() 将Long类型变成String
