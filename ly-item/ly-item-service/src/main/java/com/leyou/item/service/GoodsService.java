...
public class GoodsService {
    ...
    public void addGoods(Spu spu) {
        //添加商品要添加四个表 spu, spuDetail, sku, stock四张表 填充缺少的数据
        spu.setId(null);
    ...
    public void updateGoods(Spu spu) {
        if (spu.getId() == null) {
            throw new LyException(ExceptionEnum.GOODID_CAN_NOT_BE_NULL);//商品id不能为空
        }
        //首先查询sku
        Sku sku = new Sku();
  ...
  /*  @Transactional
    ...
    private void handleCategoryAndBrand(List<Spu> spuList) {
        for (Spu spu : spuList) {
            ...
            //根据spu中的分类ids查询分类名。利用流取出实体类中的名称，并转换为名称集合
            List<String> nameList = categoryMapper.selectByIdList(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3()))
                    .stream()
                    .map(Category::getName)//取名称，得到名称流
                    .collect(Collectors.toList());
            //对分类名进行处理
            spu.setCname(StringUtils.join(nameList, "/"));//将集合以"/" 分割后返回字符串，没有采用字符串拼接，内存占用低一些
